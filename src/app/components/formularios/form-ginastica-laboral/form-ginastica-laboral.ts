...
export class FormGinasticaLaboral implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private emailService: EmailJsService) {}
  ...
  onSubmit(): void {
    ...
    this.emailService.send('Programa de Ginástica Laboral', sanitizedData).subscribe(
      (response) => {
        this.loading = false;
        Swal.fire(
          'Sucesso! ',
